    ...
    describe(backend, () => {
      ...
      describe('directories', () => {
        ...
        it('should emit when a directory is renamed', async () => {
          ...
          if (backend === 'inotify' || backend === 'windows') {
            assert.deepEqual(res, [
              event({type: 'rename', oldPath: f1, path: f2, ino, fileId, kind}, {backend}),
            ]);
          } else {
            assert.deepEqual(res, [
              event({type: 'delete', path: f1, ino, fileId, kind}, {backend}),
      ...
      describe('sub-files', () => {
        ...
        it('should emit when a sub-file is renamed', async () => {
          ...
          if (backend === 'inotify' || backend === 'windows') {
            assert.deepEqual(res, [
              event({type: 'rename', oldPath: f2, path: f3, ino, fileId, kind}, {backend}),
            ]);
          } else {
            assert.deepEqual(res, [
      ...
      describe('readTree', () => {
        it('should store UTF-8 paths properly in the tree', async () => {
          ...
          try {
            ...
            assert.deepEqual(res, [event({type: 'delete', path: f, fileId, kind}, {backend})]);
          } finally {
